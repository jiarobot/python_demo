...
class EnhancedTopologyAnalyzer:
    ...
    def _filter_points(self, points):
        ...
        # 计算到无人机的距离
        distances = np.linalg.norm(points - self.drone_position, axis=1)
        ...
        if self.resolution > 0 and len(filtered) > 0:
            ...
            for point in filtered:
                grid_key = tuple((point // grid_size).astype(int))
    ...
    def analyze_terrain_complexity(self):
        ...
        # 从地形特征计算
        roughness = self.terrain_features['roughness']
        slope = self.terrain_features['avg_slope']
        height_range = self.terrain_features['height_range']
        obstacle_density = self.terrain_features['obstacle_density']
        traversable = self.terrain_features['traversable_area']
...
class TerrainAnalysisApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("高级地形拓扑分析系统")
    ...
    def init_ui(self):
        ...
        # 添加控制按钮
        self.load_btn = QPushButton("加载点云数据")
        ...
        vis_layout.addWidget(self.vis_3d, 1)
        ...
        vis_layout.addWidget(self.plot_2d, 1)
    ...
    def analyze_terrain(self):
        ...
        if self.analyzer.terrain_features:
            features = self.analyzer.terrain_features
            ...
            # 计算复杂度
            complexity = self.analyzer.analyze_terrain_complexity()
            self.feature_list.addItem(f"地形复杂度: {complexity:.2f}")
    ...
    def generate_simulated_terrain(self):
        ...
        zz[20:25, 15:20] += 12 + np.random.rand(5, 5) * 4  # 修改后的位置
        zz[5:10, 10:15] += 15 + np.random.rand(5, 5) * 5   # 修改后的位置
        
        # 转换为点云
        points = np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T
        
        # 添加一些随机点
        random_points = np.random.rand(500, 3) * 100
    ...
    def update_visualization(self):
        ...
        if self.analyzer.terrain_points.size > 0:
            ...
            # 障碍物
            self.plot_2d.clear()
            self.plot_2d.addItem(self.plot_2d_plot)
            self.plot_2d.addItem(self.plot_2d_drone)
            for obstacle in self.analyzer.obstacles:
                points = obstacle['points']
                if len(points) < 3:
                    continue
                ...
                try:
                    ...
                    logger.error(f"创建障碍物网格失败: {str(e)}")
                    continue

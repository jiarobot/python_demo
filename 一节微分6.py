...
class StableDifferentiableFOL(nn.Module):
    """
    稳定版可微分一阶逻辑推理系统
    解决过拟合和推理准确性问题
    """
    ...
    def _initialize_weights(self):
        """稳定的权重初始化"""
        nn.init.normal_(self.predicate_embeddings.weight, mean=0.0, std=0.01)  # 更小的标准差
    ...
    def stable_soft_forall(self, embeddings: torch.Tensor) -> torch.Tensor:
        """稳定的软全称量词"""
        # 使用更稳定的实现
        return torch.mean(embeddings, dim=1)
    
    def stable_soft_exists(self, embeddings: torch.Tensor) -> torch.Tensor:
        """稳定的软存在量词"""
        return torch.max(embeddings, dim=1)[0]
    
    def stable_and(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """稳定的逻辑与"""
        return torch.min(x, y)
    
    def stable_or(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """稳定的逻辑或"""
        return torch.max(x, y)
    
    def stable_not(self, x: torch.Tensor) -> torch.Tensor:
        """稳定的逻辑非"""
        return 1 - x
    
    def stable_implies(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """稳定的蕴含"""
        return torch.max(1 - x, y)
    
    def stable_relation_score(self, subject_emb: torch.Tensor, predicate_emb: torch.Tensor, 
        ...
        if formula_type == 'atom':
            ...
            if 'subject' in atom_data and atom_data['subject'] and 'object' in atom_data and atom_data['object']:
                ...
                try:
                    ...
                    score = self.stable_relation_score(subj_emb, pred_emb, obj_emb)
                ...
                try:
                    ...
                    result = torch.sigmoid(similarity * 3.0)  # 降低缩放因子
            ...
            if results:
                ...
                combined_result = self.stable_soft_forall(results)
            ...
            if results:
                results = torch.cat(results, dim=1)
                combined_result = self.stable_soft_exists(results)
            ...
            result = self.stable_and(left_result, right_result)
            ...
            result = self.stable_or(left_result, right_result)
            ...
            result = self.stable_not(sub_result)
            ...
            result = self.stable_implies(left_result, right_result)
...
class StableKnowledgeGraphReasoner(nn.Module):
    """
    稳定的知识图谱推理系统
    """
    ...
    def __init__(self, vocab_size: int, num_predicates: int, num_constants: int,
        ...
        # 稳定的逻辑推理层
        self.logic_reasoner = StableDifferentiableFOL(
            num_predicates, num_constants, hidden_dim // 2, 
            temperature=1.0, use_attention=False,  # 关闭注意力
...
class StableLogicReasoningSystem:
    """
    稳定逻辑推理系统 - 解决过拟合和准确性问题
    """
    ...
    def initialize_model(self):
        ...
        self.model = StableKnowledgeGraphReasoner(
            vocab_size=self.vocab_size,
            num_predicates=num_predicates,
            num_constants=num_constants,
            embedding_dim=32,  # 使用更小的维度
            hidden_dim=64,
...
# 创建稳定的训练数据
def create_stable_family_data() -> List[Dict]:
    """创建稳定的家庭关系数据"""
    samples = []
    ...
    positive_samples = [
        ...
        {
            'text': "George and Helen are siblings.",
    ...
    negative_samples = [
        ...
        {
            'text': "Michael works with Jennifer.",
            'target_relation': 'Spouse',
    ...
    # 创建训练样本
    system = StableLogicReasoningSystem("family", max_entities=8, max_seq_len=40)
...
def demo_stable_system():
    """演示稳定系统功能"""
    print("初始化稳定逻辑推理系统...")
    system = StableLogicReasoningSystem("family", max_entities=8, max_seq_len=40)
    
    print("创建稳定训练数据...")
    training_data = create_stable_family_data()
    ...
    print("开始稳定训练...")
    system.train(training_data, epochs=50, batch_size=2, learning_rate=0.0005)
    ...
    test_cases = [
        ...
        {
            'text': "Robert and Susan work together.",
            'relation': 'Sibling',
